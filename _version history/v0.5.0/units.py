...
class BasicUnit(Sprite):
    ...
    def fire(self, overwatch = False):
        ...
        if overwatch:
            self.current_ap += 1
            retain_ap = self.current_ap
        # regular shots
        else:
            self.can_fire       = False
            self.laser_charging = True
            # some classes won't lose AP after firing
            retain_ap       = self.current_ap
            self.current_ap = 0
        self._class_after_firing(retain_ap)
    ...
    # THESE TWO METHODS ARE INTENDED TO BE OVERRIDDEN BY SUBCLASSES
    def _class_after_firing(self, retain_ap):
        """plain units have no special rules after firing"""
        pass
...
class Sniper(BasicUnit):
    ...
    def _class_after_firing(self, retain_ap):
        """a sniper can no longer move, but he gets his firing APs back, minus one"""
        self.current_ap = retain_ap - 1
        # recharge his laser
        if self.laser_charging:
            self.laser_charging = False
            self.laser_uncharged = False
...
class Scout(BasicUnit):
    ...
    def _class_after_firing(self, retain_ap):
        """A scout can only fire once, but he gets his movement APs back, minus one"""
        self.current_ap = retain_ap - 1
        # reactivate movement flag
        if self.current_ap > 0:
            self.can_move = True
...
class Grunt(BasicUnit):
    """A grunt has average mobility and accuracy; it loves cover and low ground
        roll to hit : 7
        tiles per AP: 5
        EXTRA: if a normal unit gets +/-2, a Grunt gets +/-3; whether good or bad"""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _class_after_firing(self, overwatch = False):
        """grunts have no special rules after firing"""
        pass
