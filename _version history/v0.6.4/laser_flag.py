...
################################################################################
    ...
    def _prep_active_message(self, active_unit, inactive_unit = None, overwatch = False):
        ...
        if not overwatch:
            msg = self._shooter_message(active_unit)
            self.status_label[self.active_team].prep_msg(msg)
        else:
            msg = self._target_message(active_unit, inactive_unit)
        self.status_label[self.active_team].prep_msg(msg)


    def _prep_inactive_message(self, inactive_unit, active_unit, overwatch = False):
        ...
        if not overwatch:
            msg = self._target_message(inactive_unit, active_unit)
        else:
            msg = self._shooter_message(inactive_unit)
    ...
    def _shooter_message(self, shooter):
        ...
        # determine cover directions but do NOT show the roll modifier
        covered = []
        for direction, is_covered in shooter.in_cover.items():
            if is_covered:
    ...
    def _target_message(self, target, shooter):
        """create the mesage for the TARGET'S status box"""
        ...
        covered = []
        for direction, is_covered in target.in_cover.items():
            # print(direction, is_covered)
            if is_covered and direction in tile_direction(target.tile, shooter.tile):
                covered.append(direction)
        if covered:
            D = ', '.join(covered).upper()
...
################################################################################
    ...
    def _run_overwatch(self, target):
        ...
        for shooter in random_shooters:
            ...
            self._prep_active_message(target, shooter, overwatch = True)
