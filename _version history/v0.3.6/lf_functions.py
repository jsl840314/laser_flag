...
def determine_range(lf_game, unit):
    ...
    print(f"determining walking range for {unit.unit_type}...")
    ...
    unit.reachable_tiles = [[home_tile]]
    for step in range(0, unit.move_speed):
        last_step = unit.reachable_tiles[step]
        this_step = []
        for tile in last_step:
            # if climbing onto elevated ground, end here
            if tile.type == 'elevated' and not unit.elevated: continue
        ...
        unit.reachable_tiles.append(this_step)
...
def determine_los(lf_game, unit):
    """find any tiles visible to the unit"""
    print(f"determining line-of-sight for {unit.unit_type}...")

    # check each tile on the map
    unit.visible_tiles = [unit.tile]
    for target_tile in lf_game.tiles:
        if target_tile.type == 'wall':
            continue
        # determine if source has LOS to current tile
        los, sight_lines = line_of_sight(lf_game, unit.tile, target_tile)
        if los:
            unit.visible_tiles.append(target_tile)
