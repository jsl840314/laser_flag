...
units equipped with laser taggers. Each unit is one of four classes:
    Snipers are accurate shooters but slow movers
    Scouts are fast movers but poor shooters
    Grunts have average movement and aim but are affected more by terrain
        bonuses and penalties
    "Plain" units are the overall class that makes these subclasses,
        but are playable in their own right

If its line of sight is not blocked by a wall, a unit can fire its laser at
enemy units. Line of sight is not blocked by other units.

If a unit is hit by an enemy laser, he must step on a base to reactivate it.

There are two ways to win:
    OUTRIGHT: hit your team's score limit for the map
        (score limits can be different for each team to make asymmetrical games)
    CHECKMATE: occupy all bases and deactivate all enemy lasers

Several game types are then possible depending on map layout:
    Capture the flag:
        - small number of bases on each side of the map where the teams start
        - teams have enough units to cover all bases and still have free units
        - relatively high score limit so CHECKMATE is most likely win
    Battle Royale:
        - more bases on the map than either team can cover so
            OUTRIGHT is only only possible win
    King of the Hill:
        - one base in the middle, very exposed
        - low score limit so base campers will often lose OUTRIGHT before getting a CHECKMATE
    Assault:
        - one team small and elite, start near one or two bases
        - other team large and mostly plain units
        - assault team 
...
map_list = [
    'maps/capture the flag.dat',
    'maps/assault.dat',
    'maps/king of the hill.dat',
    'maps/battle royale.dat',
    'maps/_testing ground.dat'
]
current_map = map_list[4]
...
from lf_functions import Point, Line, D6, game_caption, swap, tile_direction
...
################################################################################
    ...
    def _on_click_target(self, clicked_unit):
        ...
        self._calc_to_hit(shooter=self.selected_unit, target=self.targeted_unit)
        self._prep_inactive_message(self.targeted_unit, self.selected_unit)


    def _prep_active_message(self, clicked_unit):
        plus_string = ''
        covered = []
        for direction, is_covered in clicked_unit.in_cover.items():
            if is_covered:
    ...
    def _prep_inactive_message(self, clicked_unit, shooter):
        """build the message for the target box"""
        not_string = ''
        plus_string = ''

        # determine cover directions
        covered = []
        for direction, is_covered in clicked_unit.in_cover.items():
            print(direction, is_covered)
            if is_covered and direction in tile_direction(clicked_unit.tile, shooter.tile):
                covered.append(direction)
...
################################################################################
    ...
    def _run_overwatch(self, target):
        ...
        for shooter in random_shooters:
            ...
            self._prep_active_message(target)
